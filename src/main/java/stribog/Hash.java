...
public class Hash {
    ...
    private int[] ringAdd(int[] a, int[] b) {
        ...
        for (var i = 63; i >= 0; i--) {
            ...
            c[i] = internal & 0xff;                              //поскольку в Java тип byte хранит значения от -128 до 127(первый бит отводится под знак), мы вынуждены интерпретировать значения в положительный диапазон https://habr.com/ru/post/225901/
        }
        return c;
    }
