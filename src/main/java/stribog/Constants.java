...
class Constants {
  ...
  static int[] numByte(int num) {
    var result = new int[64];
    Arrays.fill(result, 0);
    result[62] = (num * 8) >> 8;                          //исходя из того, что любое число, теоретически подаваемое в функцию, можно закодировать 2 байтам
    result[63] = (num * 8) & 0xFF;                        //поскольку в Java тип int хранит значения от -128 до 127(первый бит отводится под знак), мы вынуждены интерпретировать значения в положительный диапазон https://habr.com/ru/post/225901/
    return result;
  }
